...
- **생성 메소드 둔 이유** 
  - 서비스 쪽에서 기본 생성자 호출 후 set set set 해서 넘기게 되면, 해당 엔티티의 생성 로직이 바뀔때마다 해당 엔티티의 생성을 쓰는 곳마다 가서 다 수정 해줘야 돼. 
  - 그러나 생성 메소드를 엔티티 내에 따로 정의해두면, 생성 로직이 바뀔때 해당 메소드만 수정하면 되니까 훨씬 유지 보수가 깔끔하지.
  - 따라서, 인자를 여러개 받고 중요한 엔티티의 경우 이렇게 생성 메소드를 만들어두면 아주 좋아. 
  - 동시에 기본 메소드를 막아두는게 좋으니 protected로 정의해두거나, 롬복 이용해서  @NoArgsConstructor(access = AccessLevel.protected)를 엔티티 위에 두면 됨.
